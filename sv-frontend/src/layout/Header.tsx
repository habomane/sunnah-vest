...
function Header() {
  ...
  return (
    <nav className="mx-10 py-5 flex flex-row justify-between items-center bg-green">
        <Link to="/">
        <p className="text-3xl font-accent font-bold tracking-wide	">
          <span className=" text-green-700">Sunnah</span> <span className="text-[#dec535]">Vest</span> 
        </p>
      </Link>
      ...
      <div className={navOpen ? 'fixed inset-0 w-full h-full' : 'hidden'}>
        ...
        <div className={`absolute  duration-500 ease-out transition-all top-0 right-0 w-[45vw] h-full bg-slate-100 ${navOpen ? '': 'translate-x-full'}`}>
